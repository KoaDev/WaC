...
class MyHosts
{
    ...
    hidden [bool] IsContentInHostsFile([string]$hostsFilePath)
    {
        $systemHostsPath = Join-Path -Path $this.EtcDirectory -ChildPath 'hosts'
    ...
    [MyHosts] Get()
    {
        ...
        if ($this.HasMatchingHash($hostsFilePath, $this.Path) -and
            $this.IsContentInHostsFile($hostsFilePath))
        {
